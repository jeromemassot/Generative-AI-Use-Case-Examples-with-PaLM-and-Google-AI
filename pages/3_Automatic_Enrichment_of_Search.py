from utils import list_embedding_palm_models, create_and_fill_glossary, init_sparse_encoder
...
def list_embedding_models():
    """
    List the embedding models available in the local folder
    :return: the list of the embedding models
    """
    return list_embedding_palm_models()
...
st.write("""
    ...
    - **Step 5**: generate an answer with PaLM from these ranked contexts
    """
)
...
st.info("""
    ...
    The csv file could have 3 columns to be used for ranking the contexts
    - **column 1**: the term defined in the glossary
    - **column 2**: a short definition which can be encoded as sparse vectors
    - **column 3**: the full definition which will be enconded as dense vectors
""")
...
sparse_encode_glossary = st.sidebar.button("Init the sparse encoder from glossry")
...
embedding_models, embedding_dimensions = list_embedding_palm_models()
...
if encode_glossary and glossary is not None:
    ...
    with st.spinner("Encoding the glossary..."):
        new_index_stats = create_and_fill_glossary(
            glossary, 
